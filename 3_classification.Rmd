...
```{r}
predicted_2 <- cbind(test_data, predicted = predicted$nb.predicted)
```
...
docvars(all_corpus) <- all_posts$topic
...
uncoded_predicted <- cbind(uncoded, predicted = predict(nb_classifier, uncoded_dfm)$nb.predicted)
```
